...
# 本实现加入：每个绘图点进行多次独立随机复现实验并取均值
# 以及：修正 Fig.2(b)(c) 的非单调来源（固定超参、禁用校准、构造更干净真值、计时仅一次 _fit_once）
...
TRIES_C  = 8   # Fig.2(c) 每个 (d, N) 点的计时重复次数
...
def DRVI(
    ...
    def _fit_once(rho_, gamma0_):
        # 原先 lam_eff 随 N 增大，这里固定为 lam0，以保持不同 N 任务一致
        lam_eff = float(lam0)  # 固定岭系数；如需随 N 递减，可自行改为合适公式
...
def figure2a(seed=666):
    ...
    for rep in range(REPS_A):
        ...
        for ri, rho in enumerate(rhos):
            ...
            # Fig2(a) 不强求单调学习曲线，可保留 calibrate=True（你也可以改为 False 以完全固定）
...
# ========== Fig. 2 (c) 执行时间（秒）：每点重复计时取均值 ==========
def figure2c(seed=666):
    np.random.seed(seed)
    rho_fixed = 0.01
    d_list = [31, 61, 121]  # 与论文图一致
    N_list = [100, 200, 400, 800, 1600, 3200, 6400]
    TRIES = TRIES_C  # 为节省时间，默认稍小；需要更平滑可加大
    ...
    plt.xlabel(r"$\lg N$")
    plt.ylabel("Time (s)")
    plt.title("(c) Execution time (mean over {} runs/pt)".format(TRIES))
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()

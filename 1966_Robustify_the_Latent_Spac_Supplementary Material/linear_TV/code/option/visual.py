...
class OptionMDP():
    ...
    def step(self, action):
        ...
        return self.state, reward, util, done  # 确保返回四个值
...
def train_policy(mdp_train: OptionMDP, dataset, rho: float):
    # 获取权重和其他参数
    weights_r, weights_g, misc = DRVI(mdp_train, dataset, rho)
    # 返回 OptionLinearPolicy 对象
    return OptionLinearPolicy(mdp_train, weights_r, weights_g, misc['Lambda_inv'], misc['gamma0'], misc['beta'], misc['b'])
...
def evaluate_avg_return(env_eval: OptionMDP, policy: OptionLinearPolicy, episodes=2000, seed=None):
    ...
    for _ in range(episodes):
        ...
        for h in range(env_eval.H):
            a = policy.sample(s, h)
            s, r, util, done = env_eval.step(a)  # 确保正确解包
            ret += r
            if done:
                break
        total += ret
...
def figure2a(seed=666):
    ...
    for rho in rhos:
        ...
        pol = train_policy(mdp_train, dataset, rho)  # 直接返回 OptionLinearPolicy 对象
        policies.append(pol)
    ...
    for p0 in p0_grid:
        ...
        for idx, pol in enumerate(policies):
            # 使用 OptionLinearPolicy 中的 weights_r 和 weights_g
            pol_eval = OptionLinearPolicy(mdp_eval, pol.weights_r, pol.weights_g, pol.misc['Lambda_inv'], pol.misc['gamma0'], pol.misc['beta'], pol.misc['b'])
